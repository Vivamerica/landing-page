...
claro = ImageChops.multiply(mx.point(lambda v: 255 if 150 <= v < 245 else 0), sat.point(lambda v: 255 if v < 8 else 0))   # so cinza puro (leito/guia); hachuras coloridas ficam de fora   # cinza do leito
...
vermelho = ImageChops.multiply(ImageChops.multiply(r.point(lambda v: 255 if v > 140 else 0), ImageChops.subtract(r, g, 1, 0).point(lambda v: 255 if v >= 50 else 0)), ImageChops.subtract(r, b, 1, 0).point(lambda v: 255 if v >= 50 else 0))   # divisas em vermelho (cor 1 do CAD)
out.paste((55, 65, 81), (0, 0), ImageChops.lighter(escuro, vermelho))
